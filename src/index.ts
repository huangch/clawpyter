  ...
  api.registerTool(
    {
      ...
      async execute(_id: string, params: Record<string, unknown>) {
        ...
        if (cellIndex >= nb.cells.length) {
          return JupyterDirectClient.asToolText(
            `Overwrite cell ${cellIndex}`,
            `Cell index ${cellIndex} is out of range. Notebook has ${nb.cells.length} cells.`,
          );
        }
  ...
  api.registerTool(
    {
      ...
      async execute(_id: string, params: Record<string, unknown>) {
        ...
        if (cell.cell_type !== "code") {
          return JupyterDirectClient.asToolText(
            `Execute cell ${cellIndex}`,
            `Cell ${cellIndex} is not a code cell (type: ${cell.cell_type}).`,
          );
        }
