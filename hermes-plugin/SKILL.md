...
If the Jupyter server has `jupyter-collaboration` installed (and your
runtime has the matching optional CRDT deps — `jupyter_nbmodel_client` +
`pycrdt` on Hermes, `yjs` on OpenClaw), ClawPyter opens a shared Y.js
CRDT room for each notebook you activate. Practical consequences:
