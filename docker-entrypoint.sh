...
# Translate `JUPYTER_TOKEN` into a `--IdentityProvider.token=...` flag for the
# jupyter command. The upstream Dockerfile's CMD is the bare `jupyter lab …`
# line — it does NOT honour the env var on its own, so the only way to
# disable auth (empty string) or override the autogenerated token is to
# pass the flag on the CLI. We only inject ONCE — if the caller already
# passed `--IdentityProvider.token=…` via `docker run …`, we leave it alone.
_has_idp_flag() {
    for a in "$@"; do
        case "$a" in
            --IdentityProvider.token=*) return 0 ;;
            --IdentityProvider.token)  return 0 ;;
        esac
    done
    return 1
}
if [ -n "${JUPYTER_TOKEN+x}" ] && ! _has_idp_flag "$@"; then
